...
class SampleStates:
    ...
    def calc(self):
        ...
        for state in states:
            ...
            if test_flag:
                total_result+= str(predicted_q_values) + "\n"

        if test_flag:
            result = open("./tests/results.txt", 'w')
            result.write(total_result)
            result.close()
...
def print_final(model_name, input_file, mode, iterations, runtime):
    ...
    print "|   Model:                  " + model_name[:-3] + (" " * (28-len(model_name[:-3]))) + "|"
    print "|   Input File:             " + input_file[:-4] + (" " * (28-len(input_file[:-4]))) + "|"
    print "|   Mode:                   " + mode + (" " * 25) + "|"
    print "|   Number of iterations:   " + print_iterations + (" " * (28 - len(print_iterations))) + "|"
    print "|   Test_Mode:              " + str(test_flag) + " " * (28 - len(str(test_flag))) + "|"
    print "|" + ("_" * 55) + "|\n"
